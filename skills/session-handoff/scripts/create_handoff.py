...
HAOS adaptation:
    Handoffs are stored in memory/handoffs/, not .claude/handoffs/.
"""
...
def find_previous_handoffs(project_path: str) -> list[dict]:
    ...
    handoffs_dir = Path(project_path) / "memory" / "handoffs"
...
**Security Reminder**: Before finalizing, run `validate_handoff.py` to check for accidental secret exposure. Record env var names only, never values.
"""
